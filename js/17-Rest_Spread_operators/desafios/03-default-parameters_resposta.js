...
// 1 - Gerar um número único de 4 dígitos

// 0 <= number < 1
// console.log(Math.random());

// 1000 <= number < 100000 Com decimais
// console.log(Math.random() * 10000);

// 1000 <= number < 100000 Sem decimais
// console.log(Math.floor(Math.random() * 10000));

// 1000 <= number < 100000 Sem decimais
// console.log(1000 + Math.floor(Math.random() * 9000));

const generateTasksIds = (arrayTasks) => {

	let newTasksIds = [];

	return arrayTasks.map(({...task}) => { // {...task} copiar objeto
		if (!task.hasOwnProperty("taskId")) {
			...
			do {
				...
				newTaskId = Math.floor(1000 + Math.random() * 9000);

			} while (
				arrayTasks.find(elemento => elemento.taskId === newTaskId) ||
				newTasksIds.includes(newTaskId)
			);
...
const tasksSortedByIds = (
	arrayTasksOriginal,
	tasksWithIds = generateTasksIds(arrayTasksOriginal)
) => {
