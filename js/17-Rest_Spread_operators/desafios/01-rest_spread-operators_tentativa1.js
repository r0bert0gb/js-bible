...
const meanScore = (...argumentos) => {

	if (argumentos.some((elemento) => typeof elemento !== "number")) {

		console.log("Os argumentos fornecidos devem conter apenas números!");
		return -1;
	}

	const somaArgumentos = argumentos.reduce(
		(soma, elemento) => soma + elemento
	, 0);

	return +(somaArgumentos / argumentos.length).toFixed(2);
