...
/*
	* Plano de ação
	* 1.
	Pegar todos os argumentos e extrair todos os resultados das expressões
para um array separado. Array "vals".

Uso da variável "arguments" que só está presente em funções TRADICIONAIS.
Obtenção das expressões com o uso de "Array.from(arguments).slice(1)"

	* 2.
	"arrayOfStrings": primeiro parâmetro

	* 3.
	(arrayOfStrings.length - 1) == número de expressões (vals.length)

	* 4.
	Concatenar elementos no "arrayOfStrings" e "vals" (reduce helper method)
*/
